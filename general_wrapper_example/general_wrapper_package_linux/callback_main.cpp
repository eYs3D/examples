...
int TransformDepthDataType(int depth_raw_data_type , int bRectifyMode , unsigned short wPID , int depthHeight)
{
    ...
    {
        if(wPID == 0x0120 && depthHeight == 360)
        {
...
printf("========================Filter result: %d========================\n",mIsdepthfilterEnable); 
	...
	if(mode_config.vecDepthType.size() == 0)
		...
		depth_raw_data_type = TransformDepthDataType(mode_config.vecDepthType.at(depth_raw_index),mode_config.bRectifyMode,mCameraDeviceInfo.devInfo.wPID,config.depthHeight);
	}
...
//                                 DEPTH_IMG_NON_TRANSFER,
        ...
        ret = device->initStream(libeYs3D::video::COLOR_RAW_DATA_TYPE::COLOR_RAW_DATA_YUY2,
                                 2560, 720, 60,
                                 libeYs3D::video::DEPTH_RAW_DATA_TYPE::ETronDI_DEPTH_DATA_OFF_RAW,
                                 0, 0,
//                                 DEPTH_IMG_NON_TRANSFER,
                                 DEPTH_IMG_COLORFUL_TRANSFER,
                                 IMAGE_SN_SYNC,
